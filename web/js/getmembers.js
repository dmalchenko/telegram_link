VK.init({
    apiId: 6212174 // ID вашего приложения VK
});

var membersGroups = []; // массив участников группы
getMembers(77253035, 5000);

function getMembers(groupId) {
    VK.Api.call('groups.getMembers', {group_id: groupId, v: "5.73", sort: 'id_asc'}, function (r) {
        $.ajax({
            type: "POST",
            url: "/vk/load-members",
            data: {
...
// function getMembers(group_id, members_count) {
//     var code = 'var members = API.groups.getMembers({"group_id": ' + group_id + ', "v": "5.74", "sort": "id_asc", "count": "1000", "offset": ' + membersGroups.length + '}).items;' // делаем первый запрос и создаем массив
...
//         + '{'
//         + 'members = members + "," + API.groups.getMembers({"group_id": ' + group_id + ', "v": "5.74", "sort": "id_asc", "count": "1000", "offset": (' + membersGroups.length + ' + offset)}).items;' // сдвиг участников на offset + мощность массива
